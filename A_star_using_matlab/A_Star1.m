...
uicontrol('str','path','call',{@Algorthimm})
...
xlabel('Select the Target ','Color','black');
but=0;
...
h=msgbox('Select Obstacles using the Left Mouse button,to select the last obstacle use the Right button');
  xlabel('Select Obstacles','Color','blue');
uiwait(h,10);
if ishandle(h) == 1
    delete(h);
end
...
xlabel('initial position','Color','black');
but=0;
while (but ~= 1) %Repeat until the Left button is not clicked
    [xval,yval,but]=ginput(1);
    xval=floor(xval);
...
but=0;
while (but ~= 1) %Repeat until the Left button is not clicked
    [xval,zval,but]=ginput(1);
end
xval=floor(xval);
zval=floor(zval);
xTarget=xval;%X Coordinate of the Target
zTarget=zval;%Y Coordinate of the Target

MAP(xval,zval)=0;%Initialize MAP with location of the target
plot(xval+.5,zval+.5,'gd');
text(xval+1,zval+.5,'Target')
...
while but == 1
    ...
    xval=floor(xval);
    zval=floor(zval);
    MAP(xval,zval)=-1;%Put on the closed list as well
    plot(xval+.5,zval+.5,'ro');
 end%End of While loop
...
xlabel('Select the initial position ','Color','black');
but=0;
while (but ~= 1) %Repeat until the Left button is not clicked
    [xval,yval,but]=ginput(1);
    xval=floor(xval);
...
MAP(xval,zval)=1;
 plot(xval+.5,zval+.5,'bo');
%End of obstacle-Target pickup



function Algorthimm(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
